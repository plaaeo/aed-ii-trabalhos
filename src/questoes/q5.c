...
static bool satisfaz_condicao(float coeficiente, float valor, char operacao) {
    switch (operacao) {
        case '>':  return coeficiente > valor;
        case '<':  return coeficiente < valor;
        case '>=': return coeficiente >= valor;
        case '<=': return coeficiente <= valor;
...
void q5(FILE *arquivo, float buscas[], size_t n_buscas, char operacao) {
    ...
    for (size_t i = 0; i < n_buscas; i++) {
        ...
        while (fread(&aluno, sizeof(aluno_t), 1, arquivo) == 1) {
            if (satisfaz_condicao(aluno.coeficiente, buscas[i], operacao)) {
                encontrados[i]++;
            }
        }
